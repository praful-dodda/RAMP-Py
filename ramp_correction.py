...
def interpolate_ramp(decile_bounds, curve, current_model_val, avg_slope=None):
    """
    ...
    Args:
        ...
        avg_slope (float, optional): The average slope for linear extrapolation.
    ...
    for t in range(num_months):
        ...
        for p in range(num_grid_points):
            ...
            if is_out_of_bounds:
                ...
                lambda2_df.loc[p, dma_col] = max(0, l2) if not np.isnan(l2) else 0.01
            ...
            # ################################################
            # ### START: INSERT THIS NEW QA BLOCK HERE ###
            # ################################################
            if (month == 1 or month == 7) and p in plot_indices:
                print(f"\n--- QA for Point {p}, Month {month} ---")
                print(f"Current Model Value at this grid point: {current_model_val:.2f}")
                if len(valid_local_bounds) > 1:
                    min_b, max_b = valid_local_bounds.min(), valid_local_bounds.max()
                    print(f"Local RAMP curve bounds (from neighbors): Min={min_b:.2f}, Max={max_b:.2f}")
                    if current_model_val >= min_b and current_model_val <= max_b:
                        print("Result: SUCCESS - Value is WITHIN local bounds.")
                    else:
                        print("Result: FAILURE - Value is OUTSIDE local bounds.")
                else:
                    print("Result: FAILURE - Not enough local data to create bounds.")
            # ##############################################
            # ### END: END OF THE QA BLOCK             ###
            # ##############################################
...
if __name__ == '__main__':
    ...
    print("\nCheck 'ramp_plots' directory for diagnostic images.")
