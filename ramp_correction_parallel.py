...
def enforce_monotonicity(lambda_curve):
    ...
    if np.isnan(mean_val): return monotonic_lambda
    ...
    for i in range(middle_bin + 1, total_bins):
        ...
        if not np.isnan(prev_val) and monotonic_lambda[i] < prev_val: monotonic_lambda[i] = prev_val
...
def process_grid_point(p, model_grid_df, dma_col, global_bounds, global_curve1, global_curve2, obs_kdtree, collocated_df, time_window_months):
    ...
    # --- Early exit check (same as before) ---
    ...
    if len(valid_global_bounds) > 1 and (current_model_val < valid_global_bounds.min() or current_model_val > valid_global_bounds.max()):
        ...
        final_l2 = max(0, l2) if not np.isnan(l2) else 0.01
        return p, l1, final_l2, GLOBAL_FALLBACK_CODE
    ...
    local_bounds, local_curve1, local_curve2, _ = compute_decile_curves(local_model_vals, local_obs_vals, TOTAL_BINS)
    ...
    for i in range(MAX_ADJUSTMENTS):
        ...
        adj_bounds, adj_curve1, adj_curve2, _ = compute_decile_curves(adj_model_vals, adj_obs_vals, TOTAL_BINS)
...
# --- MODIFIED: Main RAMP Correction Function ---
def get_overhang_ramp(collocated_df, model_grid_df, results_dir=None, num_cores=None):
    ...
    obs_coords = collocated_df[['lon_toar', 'lat_toar']].drop_duplicates().values
    obs_kdtree = cKDTree(obs_coords)
    
    # --- Main Time Loop (This part remains serial) ---
    for t in range(num_months):
        ...
        # Global RAMP calculation is done once per month
        ...
        global_bounds, global_curve1, global_curve2, global_bin_edges = compute_decile_curves(global_model_vals, global_obs_vals)
        ...
        if plotting_enabled:
            plot_global_ramp(global_bounds, global_curve1, global_curve2, global_model_vals, global_obs_vals, global_bin_edges, month, results_dir)

        # --- Parallel Grid Point Processing ---
        with Pool(processes=num_cores) as pool:
            # Prepare arguments for each worker process
            # Use partial to "freeze" arguments that are the same for all workers
            ...
            # Run the processes. The iterable is just the list of grid point indices.
...
# --- Plotting Functions (Unchanged, local plotting is now unused) ---
